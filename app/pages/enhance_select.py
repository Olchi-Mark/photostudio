...
import os, re, shutil, datetime as dt, logging, inspect
...
# ────────────────────────────────────────────────────────────────────────────────
# 이메일 전송 호환 헬퍼 (구/신 emailer 모두 지원)
# ────────────────────────────────────────────────────────────────────────────────
def _send_email_compat(to_addr: str, subject: str, body: str, attachments: List[str], email_cfg: dict):
    """
    - 신버전 emailer: send_email_checked(to_email=..., cfg=...) 또는 send_email(to_email=..., cfg=...)
    - 구버전 emailer: send_email(to=..., config=...)
    둘 다 자동 지원.
    """
    # 1) send_email_checked가 있으면 최우선
    fn_checked = getattr(emailer, "send_email_checked", None)
    if callable(fn_checked):
        try:
            return fn_checked(to_email=to_addr, subject=subject, body=body, attachments=attachments, cfg=email_cfg)
        except TypeError:
            # 드물게 시그니처가 다른 포크 대비
            pass

    # 2) send_email 존재 → 파라미터 이름을 보고 분기
    fn_send = getattr(emailer, "send_email", None)
    if not callable(fn_send):
        raise AttributeError("emailer.send_email 함수가 없습니다.")

    try:
        sig = inspect.signature(fn_send)
        params = sig.parameters
        if "to_email" in params:  # 신버전
            return fn_send(to_email=to_addr, subject=subject, body=body, attachments=attachments, cfg=email_cfg)
        elif "to" in params:      # 구버전
            # 구버전은 config 키 이름이 'config' 또는 'cfg'일 수 있음 → 우선순위 처리
            kwargs = dict(to=to_addr, subject=subject, body=body, attachments=attachments)
            if "config" in params:
                kwargs["config"] = email_cfg
            else:
                kwargs["cfg"] = email_cfg
            return fn_send(**kwargs)
        else:
            # 예외적인 포크: 위치인자만 받는 경우가 드물게 존재할 수 있음
            return fn_send(to_addr, subject, body, attachments, email_cfg)
    except Exception as e:
        _log(f"EmailCompat:error {e}")
        raise
...
class EnhanceSelectPage(BasePage):
    ...
    def _on_finish(self):
        ...
        def work(update):
            ...
            email_cfg = emailer.load_email_config()  # 정규화된 이메일 설정
                        ...
                        if rec_p:
                            ...
                            for to in rec_p:
                                try:
                                    ...
                                    _log(f"Email:printer to={to} result={getattr(res, 'ok', True)}")
                                except Exception as e:
                    ...
                    if rec_r:
                        ...
                        for to in rec_r:
                            try:
                                ...
                                _log(f"Email:retouch to={to} result={getattr(res, 'ok', True)}")
                            except Exception as e:
