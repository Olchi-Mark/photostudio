"""
app/ui/ai_overlay2.py — 안정화 버전(새 파일)
역할: 라이브뷰 위에 가이드/마스크/랜드마크를 그리는 오버레이 위젯(페인터 안정화)

사용: 기존 ai_overlay.py 대체용으로 임시 제공. 호출부에서 모듈만 바꾸면 동작하도록
      API를 최대한 유지했다.
"""
...
class OverlayCanvas2(QWidget):
    """카메라 프리뷰 위에 가이드를 그리는 페인트 전용 위젯(안정화)."""
...
class AiOverlay2(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        ...
        lay.setSpacing(0)
        self._overlay = OverlayCanvas2(self)
